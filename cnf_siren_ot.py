...
# Set CNF_INSTALL_DEPS=1 to install the pinned stack (numpy 1.26.4, jax 0.4.35,
# equinox 0.11.8, optax 0.2.3, diffrax 0.6.0) on a fresh Colab runtime and exit.
# Off by default so it never wrecks a working environment.  Also runs fine on
# current versions (tested on jax 0.10 / diffrax 0.7).
if os.environ.get("CNF_INSTALL_DEPS", "0") == "1":
    import subprocess
    import sys
    ...
    subprocess.check_call([sys.executable, "-m", "pip", "install", "--no-cache-dir",
                           "numpy==1.26.4", "jax==0.4.35", "jaxlib==0.4.35",
                           "equinox==0.11.8", "optax==0.2.3", "diffrax==0.6.0",
                           "matplotlib"])
    print("Installed. Restart the runtime and run again.", flush=True)
    sys.exit(0)
...
class Config:
    ...
    width_size: int = 128
    depth: int = 3
    w0_first: float = 30.0        # frequency scale of layer 1 == stiffness at init
    w0_hidden: float = 1.0

    # Penalties
    kinetic_alpha: float = 0.1    # OT transport cost (Benamou-Brenier)
    jacobian_alpha: float = 0.0   # RNODE Frobenius term
    ...
    def summary(self):
        return (f"a_ke={self.kinetic_alpha:<8.4g} a_jac={self.jacobian_alpha:<8.4g} "
                ...
                f"lr={self.learning_rate:.2e} {self.lr_schedule}")



# 2. Data -- FFJORD checkerboard, with a train/val/test split.

def make_checkerboard(num_samples, rng):
    """8 alternating uniform squares tiling [-4, 4]^2 (same code as before)."""
...
class Datasets:
    def __init__(self, n_train=10_000, n_val=2_000, n_test=2_000, seed=42):
        ...
        test = make_checkerboard(n_test, np.random.default_rng(123))  # seed of earlier runs
...
# 3. SIREN vector field.

class SirenLayer(eqx.Module):
    """x -> sin(w0 * (W x + b)), with the SIREN-paper init.
    First layer:  W ~ U(-1/in, 1/in);  later: W ~ U(-sqrt(6/in)/w0, +...)."""
...
class SirenField(eqx.Module):
    """The CNF vector field v(t, y) = SIREN([y, t])."""
...
# 4. Augmented dynamics: log-density + OT transport cost + Frobenius term.
...
def exact_dynamics(t, state, args):
    """Exact Jacobian trace (cheap in 2D)."""
    y, _, _, _ = state
    (func,) = args
    f, vjp_fn = jax.vjp(lambda z: func(t, z, None), y)
    (jac,) = jax.vmap(vjp_fn)(jnp.eye(y.shape[0]))   # row i = d f_i / d y
    return f, jnp.trace(jac), 0.5 * jnp.sum(f**2), jnp.sum(jac**2)


def approx_dynamics(t, state, args):
    """Hutchinson's estimator, for higher dimensions.
    E[eps^T J eps] = tr(J) and E[||eps^T J||^2] = ||J||_F^2, so both stay unbiased."""
    y, _, _, _ = state
    eps, func = args
    f, vjp_fn = jax.vjp(lambda z: func(t, z, None), y)
    (eps_jac,) = vjp_fn(eps)
...
class CNF(eqx.Module):
    ...
    def _solve(self, y, *, key, forward):
        """Returns (y_end, delta_logp, kinetic, jac_frob, nfe, num_steps)."""
        if self.exact_logp:
            term, args = diffrax.ODETerm(exact_dynamics), (self.func,)
        else:
            eps = jr.normal(key, (self.data_size,))
            term, args = diffrax.ODETerm(approx_dynamics), (eps, self.func)
        ...
        if nfe is None:
            nfe = 6 * num_steps + 1  # Tsit5: 7 stages, FSAL
        return y_final, delta_logp, ke, jf, nfe, num_steps

    def sample(self, *, key):
        noise_key, solve_key = jr.split(key)
        z0 = jr.normal(noise_key, (self.data_size,))
        y, _, _, _, nfe, _ = self._solve(z0, key=solve_key, forward=True)
        return y, nfe

    def log_prob(self, y, *, key):
        latent, dlogp, _, _, nfe, _ = self._solve(y, key=key, forward=False)
...
def batch_metrics(model: CNF, data, key) -> Metrics:
    ...
    latents, dlogps, kes, jfs, nfes, steps = jax.vmap(
        lambda x, k: model._solve(x, key=k, forward=False)
    )(data, keys)
...
def regularized_loss(model: CNF, data, key, cfg: Config):
    """L = NLL + alpha_ke * transport_cost + alpha_jac * ||J||_F^2."""
...
def train(cfg: Config, loader, key, *, until_step: int,
    ...
    """Train (or continue training) to `until_step` total steps."""
    if state is None:
    ...
    for step in range(state.steps_done, until_step):
        ...
        if not np.isfinite(nll):
            # A diverged config is information (that region of the space is bad),
            # not a crash.  Prune it and move on.
...
def evaluate(model: CNF, data, key, chunk: int = 500) -> dict:
    """Held-out metrics, in fixed-size chunks (one compile, bounded memory)."""
...
SEARCH_SPACE = {
    "kinetic_alpha": LogUniform(1e-3, 1.0),     # the OT transport-cost weight
    "jacobian_alpha": LogUniform(1e-4, 1e-1),   # the RNODE stiffness weight
...
def seed_configs(base: Config):
    """Reference points that should always be on the leaderboard."""
    return [
        base.replace(kinetic_alpha=0.0, jacobian_alpha=0.0),                  # plain SIREN
        base.replace(kinetic_alpha=0.1, jacobian_alpha=0.0),                  # the KE script
        base.replace(kinetic_alpha=0.1, jacobian_alpha=0.01, w0_first=10.0),  # + jac, softer w0
        base.replace(kinetic_alpha=1.0, jacobian_alpha=0.01, w0_first=5.0),   # strong
    ]


def score(nll, nfe, nfe_weight):
    """Lower is better.  nfe_weight prices one solver evaluation in nats: the default
    0.002 says '50 NFE is worth 0.1 nats'.  Diverged trials score +inf."""
...
def is_feasible(nll, nfe):
    """Beat BOTH baselines at once -- the stated goal."""
...
def time_per_iter(cfg, loader, key, n=3):
    """Seconds per step AFTER compilation -- for an honest up-front cost estimate."""
    state = init_state(cfg, key)
    train(cfg, loader, key, until_step=1, state=state)   # absorb the JIT compile
    t0 = time.time()
...
def run_search(args, data):
    ...
    configs = seed_configs(base) + [
        base.replace(**{k: v.sample(rng) for k, v in SEARCH_SPACE.items()})
    ...
    print(f"  total {total:,} iters ~= {total * spi / 3600:.1f} h "
          f"(one full 2000-iter run at batch 512 was 8.6 h)", flush=True)
    ...
    for si, target in enumerate(stages):
        ...
        for tr in trials:
            ...
            rec = {"trial": tr["id"], "stage": si, "steps": st.steps_done,
                   ...
                   "train_time_s": st.train_time, "is_final_stage": False}
            records.append(rec)
            stage_recs.append(rec)
            with open(jsonl, "a") as f:      # crash-safe: written as we go
...
def plot_run(model, cfg, test_data, state, path, n_generate=500):
    ...
    ax.set_xlabel("Iteration"); ax.set_ylabel("Loss (nats)")
    ...
    ax2.set_ylabel("Transport cost (KE)", color="green")
    ax.set_xlabel("Iteration"); ax.set_ylabel("NFE")
    ax.set_title("Solver cost & OT transport cost")
    ...
    ax.set_xlabel("NFE"); ax.set_ylabel("NLL")
    ax.set_title("NLL vs NFE (dashed = tanh baseline)"); ax.grid(True, alpha=0.3)

    test_np = np.asarray(test_data)
    axes[1, 0].scatter(test_np[:, 0], test_np[:, 1], s=5, alpha=0.5)
    axes[1, 0].set_aspect("equal"); axes[1, 0].set_title("True checkerboard (test)")
    ...
    axes[1, 2].set_aspect("equal"); axes[1, 2].legend(fontsize=8)
...
def plot_search(records, nfe_weight, path):
    ...
    ax.set_xlabel("Validation NFE"); ax.set_ylabel("Validation NLL (nats)")
    ax.set_title("Search: NLL / NFE trade-off\n(bottom-left quadrant beats both baselines)")
    ...
    ax.set_xlabel("kinetic_alpha (OT transport-cost weight)")
    ax.set_ylabel(f"score = NLL + {nfe_weight}*NFE")
    ax.set_title("Does the transport-cost penalty pay off?"); ax.grid(True, alpha=0.3)
    ...
    ax.set_xlabel("w0_first (SIREN stiffness at init)"); ax.set_ylabel("Validation NFE")
    ax.set_title("Solver cost vs SIREN frequency scale"); ax.grid(True, alpha=0.3)

    ax = axes[1, 1]
    ax.axis("off")
    ranked = sorted(final, key=lambda r: score(r["val_nll"], r["val_nfe"], nfe_weight))[:8]
    rows = [["#", "a_ke", "a_jac", "w0", "w/d", "lr", "NLL", "NFE", "score"]]
...
def parse_args():
    ...
    p.add_argument("--tune", action="store_true", help="run the hyperparameter search")
    p.add_argument("--smoke", action="store_true", help="tiny budgets; check it runs")
    p.add_argument("--out", default="results_ot")

    # single-run overrides
    p.add_argument("--config", default=None, help="path to a JSON config to load")
    ...
    p.add_argument("--time-budget", type=float, default=float("inf"), help="hours")
...
def single_run_config(args) -> Config:
    ...
    return cfg.replace(**{k: v for k, v in over.items() if v is not None})
...
def report(tag, ev):
    ...
    print("RESULT: " + ("beats BOTH baselines (better NLL and cheaper solver)."
...
def main():
    ...
    print("=" * 78, flush=True)
    print("CNF + SIREN + OT transport-cost penalty -- 2D checkerboard", flush=True)
    ...
    if not args.tune:
        ...
        print(f"        iters={cfg.num_iters}, batch={cfg.batch_size}, "
              f"rtol=atol={cfg.rtol}, exact_logp={cfg.exact_logp}", flush=True)
    ...
    if args.no_final:
        print(f"\n--no-final: retrain later with\n"
              f"    python cnf_siren_ot.py --config {args.out}/best_config.json", flush=True)
        return

    # -------- final: retrain the winner at full budget, touch test once --------
    print("\n" + "=" * 78, flush=True)
    ...
    with open(os.path.join(args.out, "final_result.json"), "w") as f:
        json.dump({"config": fcfg.to_dict(), "test": test, "search_hours": hours}, f, indent=2)
